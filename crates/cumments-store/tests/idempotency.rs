...
use cumments_core::{
    ...
    ports::{IdempotencyInput, IdempotencyOutcome, IntentStore},
};
...
#[tokio::test]
async fn same_key_and_fingerprint_replays_original_intent() {
    let store = DbStore::connect(&test_db_url("replay"))
        .await
        .expect("connect db");

    let first = store
        .save_post_intent_idempotent(&post_intent(), &input("retry-me-123", "fingerprint-a"))
        .await
        .expect("first submit");
    let IdempotencyOutcome::Accepted { intent_id } = first else {
    ...
    assert_eq!(
        replay,
        IdempotencyOutcome::Replayed { intent_id },
        "identical retry must return the original intent"
    );

    // Only one intent was queued.
...
async fn same_key_with_different_fingerprint_is_reused() {
    ...
    let reused = store
        .save_post_intent_idempotent(&post_intent(), &input("retry-me-123", "fingerprint-b"))
        .await
        .expect("second submit");
    assert_eq!(reused, IdempotencyOutcome::Reused);

    // The rejected request must not have queued an intent.
    let pending = store.get_pending_post_intents(100).await.expect("pending");
    assert_eq!(pending.len(), 1);
}
...
async fn expired_keys_are_purged_and_can_be_reused() {
    ...
    let outcome = store
        .save_post_intent_idempotent(&post_intent(), &input("stale-key-123", "fingerprint-new"))
        .await
    ...
    // The stale row is gone and exactly one intent is queued.
...
async fn same_key_from_different_authors_is_independent() {
    ...
    let first = store
        .save_post_intent_idempotent(&post_intent(), &input("shared-key-123", "fingerprint-a"))
        .await
        .expect("author A submit");
    let second = store
        .save_post_intent_idempotent(&other, &other_input)
        .await
...
#[tokio::test]
async fn concurrent_identical_submissions_queue_only_one_intent() {
    ...
    for handle in handles {
        match handle.await.expect("join task") {
            IdempotencyOutcome::Accepted { intent_id } => accepted.push(intent_id),
            IdempotencyOutcome::Replayed { intent_id } => replayed.push(intent_id),
    ...
    assert!(
        ...
        "all replays must return the winner's intent"
    );

    let pending = store.get_pending_post_intents(100).await.expect("pending");
    assert_eq!(pending.len(), 1, "duplicate intents must not be queued");
